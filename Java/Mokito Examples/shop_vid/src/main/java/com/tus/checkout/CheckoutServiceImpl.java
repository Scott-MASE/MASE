    ...
    public void checkout(Customer customer) throws CartException {
        ...
        for (CartItemDTO item : cart.getItems()) {
            if (!inventoryService.isAvailable(item.getProductId(), item.getQuantity())) {
                throw new InventoryException(
                        "Product " + item.getProductName() + " is out of stock or doesn't have sufficient quantity.");
            }
        }
        ...
        try {
            if (customer.getPaymentType().equals("CreditCard")) {
                ...
                paymentGateway.processCreditCard(totalAmount, creditCardStrategy.getCardNumber(),
                        creditCardStrategy.getExpiryDate(), creditCardStrategy.getCvv());
            } else if (customer.getPaymentType().equals("PayPal")) {
